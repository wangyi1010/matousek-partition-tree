...
from matousek_partition_tree import line_crosses_tri, simplicial_partition
...
def main() -> None:
    ...
    print(f"n={n}, r={R}, s={s}: {len(part)} groups, "
    ...
    for k, (group, _) in enumerate(part):
        ...
        ax1.scatter([float(p[0]) for p in group],
                    [float(p[1]) for p in group], s=7, color=col,
                    linewidths=0)
    ax1.set_title(f"{len(part)} disjoint point groups, sizes in "
                  f"[{s}, {2 * s})", fontsize=11)

    # panel 2: a few sample groups with their simplices
    show = rng.sample(range(len(part)), 6)
    sample_cmap = plt.get_cmap("tab10")
    ax2.scatter([float(p[0]) for p in pts], [float(p[1]) for p in pts],
                s=4, color="0.82", linewidths=0)
    for j, k in enumerate(show):
        group, tri = part[k]
        col = sample_cmap(j)
        tri_f = [(float(v[0]), float(v[1])) for v in tri]
        ax2.add_patch(MplPolygon(tri_f, closed=True, facecolor=col,
                                 alpha=0.13, edgecolor=col, linewidth=1.4))
        ax2.scatter([float(p[0]) for p in group],
                    [float(p[1]) for p in group], s=10, color=col,
                    linewidths=0)
    ax2.set_title("6 sample groups and their simplices (clipped to view;\n"
    ...
    for (group, _), cr in zip(part, crossed):
        col, size = ("#d85a30", 8) if cr else ("0.78", 4)
        ax3.scatter([float(p[0]) for p in group],
                    [float(p[1]) for p in group], s=size, color=col,
                    linewidths=0)
    xs = [-0.06, 1.06]
    ax3.plot(xs, [float(h[0]) * x + float(h[1]) for x in xs], "k-",
             linewidth=1.6)
    ax3.set_title(f"query line crosses {n_crossed}/{len(part)} simplices\n"
                  "orange points: must recurse — gray: counted wholesale",
                  fontsize=11)
    ax3.legend(handles=[
        Line2D([], [], color="k", linewidth=1.6, label="query line h"),
        Line2D([], [], marker="o", linestyle="", color="#d85a30",
               markersize=5, label="in crossed simplex"),
        Line2D([], [], marker="o", linestyle="", color="0.78",
               markersize=5, label="resolved in O(1)")],
        loc="lower right", fontsize=8)

    fig.suptitle(
        "Verified Matousek simplicial partition — one application of the "
        f"Partition Theorem (n={n}, r={R})", fontsize=12.5)
