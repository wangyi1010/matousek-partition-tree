...
\else % if luatex or xetex
  ...
  \fi
  \defaultfontfeatures{Scale=MatchLowercase,Ligatures={TeX,NoCommon}}
  \defaultfontfeatures[\rmfamily]{Ligatures={TeX,NoCommon},Scale=1}
  \setmainfont[Ligatures={TeX,NoCommon}]{Helvetica Neue}
  \setsansfont[Ligatures={TeX,NoCommon}]{Helvetica Neue}
...
Source: planar specialization of Matoušek's Partition Theorem from
...
It explains why the cutting tool is valid and why the two-dimensional
complexity is $O(t^2)$. The sharp optimal version relies on the
exponential decay estimate above, which is the Chazelle-Friedman
sampling argument cited in the references.
...
\tightlist
\item
  Jiří Matoušek, \textbf{Efficient Partition Trees}, \emph{Discrete
